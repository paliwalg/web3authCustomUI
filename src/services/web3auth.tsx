...
export interface IWeb3AuthContext {
  ...
  createPaymentTxRamp: () => Promise<any>;
  createPaymentTxWyre: () => Promise<any>;
...
export const Web3AuthContext = createContext<IWeb3AuthContext>({
  ...
  getAccounts: async () => {},
  getBalance: async () => {},
  createPaymentTxRamp: async () => {}, 
  createPaymentTxWyre: async () => {}, 
});
...
export const Web3AuthProvider: FunctionComponent<IWeb3AuthState> = ({ children, web3AuthNetwork, chain }: IWeb3AuthProps) => {
  ...
  const createPaymentTxRamp = async () => {
    try {
      // const { torus } = web3Obj;
      if (!provider) {
        console.log("provider not initialized yet");
      ...
      const res = await torus.initiateTopup("rampnetwork", {
        selectedAddress: accounts[0],
        selectedCurrency: "USD",
        selectedCryptoCurrency: "MATIC_USDC",
      })
      console.log(res)
    } catch (error) {
      console.error(error)
  ...
  const createPaymentTxWyre = async () => {
    try {
      // const { torus } = web3Obj;
      if (!provider) {
        console.log("provider not initialized yet");
      ...
      const res = await torus.initiateTopup("wyre", {
        selectedAddress: accounts[0],
        selectedCurrency: "USD",
        selectedCryptoCurrency: "MUSDC",
      })
      console.log(res)
    } catch (error) {
      console.error(error)
